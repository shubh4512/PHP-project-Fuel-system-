...
if(isset($_POST['submit'])){
    ...
    if($date0&&$reason0&&$amount0){
        pg_query($con,"insert into debit (date,reason,amount,users)  values('$date0','$reason0',$amount0,'$user')");
        echo "<tr><td>$date0</td><td>$reason0</td><td>$amount0</td></tr>" ;
        $check=1;
    }
    if($date1&&$reason1&&$amount1){
        pg_query($con,"insert into debit (date,reason,amount,users)  values('$date1','$reason1',$amount1,'$user')");
        echo "<tr><td>$date1</td><td>$reason1</td><td>$amount1</td></tr>" ;
        $check=1;
    }
    if($date2&&$reason2&&$amount2){
        pg_query($con,"insert into debit (date,reason,amount,users)  values('$date2','$reason2',$amount2,'$user')");
        echo "<tr><td>$date2</td><td>$reason2</td><td>$amount2</td></tr>" ;
    }
    if($date3&&$reason3&&$amount3){
        pg_query($con,"insert into debit (date,reason,amount,users)  values('$date3','$reason3',$amount3,'$user')");
        echo "<tr><td>$date3</td><td>$reason3</td><td>$amount3</td></tr>" ;
        $check=1;
    }
    if($date4&&$reason4&&$amount0){
        pg_query($con,"insert into debit (date,reason,amount,users)  values('$date4','$reason4',$amount4,'$user')");
        echo "<tr><td>$date4</td><td>$reason4</td><td>$amount4</td></tr>" ;
        $check=1;
    }
    if($date5&&$reason5&&$amount5){
        pg_query($con,"insert into debit (date,reason,amount,users)  values('$date5','$reason5',$amount5,'$user')");
        echo "<tr><td>$date5</td><td>$reason5</td><td>$amount5</td></tr>" ;
        $check=1;
    }
    if($date6&&$reason6&&$amount6){
        pg_query($con,"insert into debit (date,reason,amount,users)  values('$date6','$reason6',$amount6,'$user')");
        echo "<tr><td>$date6</td><td>$reason6</td><td>$amount6</td></tr>" ;
        $check=1;
    }
    if($date7&&$reason7&&$amount7){
        pg_query($con,"insert into debit (date,reason,amount,users)  values('$date7','$reason7',$amount7,'$user')");
...
?>
            ...
            <h2>VIEW</h2><br>
                ...
                $con=pg_connect("host=localhost dbname=groups user=postgres password=postgres");
                    ...
                    $res=pg_query($con,"select distinct (reason) from debit where users='$user'");
...
if($reason=='all'){
    $res=pg_query($con,"select * from debit where date between '$start' and '$end' and users='$user'");
...
$res=pg_query($con,"select * from debit where date between '$start' and '$end' and reason='$reason' and users='$user'");
echo'<table border="1" class="stable"><tr><th>Date</th><th>Reason</th><th>Amount</th></tr>';
                    while($row=pg_fetch_array($res)){
                        $viewdate=date("d-m-Y",strtotime($row[0]));
                        echo "<tr><td>$viewdate</td><td>$row[1]</td><td>$row[2]</td></tr>";  
