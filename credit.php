...
if(isset($_POST['submit'])){
    ...
    if($date0&&$purpose0&&$type0&&$amount0){
        pg_query($con,"insert into credit (date,purpose,type,amount,users,quantity)  values('$date0','$purpose0','$type0',$amount0,'$user',$quantity0)");
        echo "<tr><td>$date0</td> <td>$purpose0</td> <td>$type0</td> <td>$quantity0</td> <td>$amount0</td> </tr>" ;
        $check=1;
    }
    if($date1&&$purpose2&&$type1&&$amount1){
        pg_query($con,"insert into credit (date,purpose,type,amount,users,quantity) values('$date1','$purpose1','$type1',$amount1,'$user',$quantity1)");
        echo "<tr><td>$date1</td> <td>$purpose1</td> <td>$type1</td> <td>$quantity1</td> <td>$amount1</td> </tr>" ;
        $check=1;
    }
    if($date2&&$purpose2&&$type2&&$amount2){
        pg_query($con,"insert into credit (date,purpose,type,amount,users,quantity) values('$date2','$purpose2','$type2',$amount2,'$user',$quantity2)");
        echo "<tr><td>$date2</td> <td>$purpose2</td> <td>$type2</td> <td>$quantity2</td> <td>$amount2</td> </tr>" ;
        $check=1;
    }
    if($date3&&$purpose3&&$type3&&$amount3){
        pg_query($con,"insert into credit (date,purpose,type,amount,users,quantity) values('$date3','$purpose3','$type3',$amount3,'$user',$quantity3)");
        echo "<tr><td>$date3</td> <td>$purpose3</td> <td>$type3</td> <td>$quantity3</td> <td>$amount3</td> </tr>" ;
        $check=1;
    }
    if($date4&&$purpose4&&$type4&&$amount4){
        pg_query($con,"insert into credit (date,purpose,type,amount,users,quantity) values('$date4','$purpose4','$type4',$amount4,'$user',$quantity4)");
        echo "<tr><td>$date4</td> <td>$purpose4</td> <td>$type4</td> <td>$quantity4</td> <td>$amount4</td> </tr>" ;
        $check=1;
    }
    if($date5&&$purpose5&&$type5&&$amount5){
        pg_query($con,"insert into credit (date,purpose,type,amount,users,quantity) values('$date5','$purpose5','$type5',$amount5,'$user',$quantity5)");
        echo "<tr><td>$date5</td> <td>$purpose5</td> <td>$type5</td> <td>$quantity5</td> <td>$amount5</td> </tr>" ;
        $check=1;
    }
    if($date6&&$purpose6&&$type6&&$amount6){
        pg_query($con,"insert into credit (date,purpose,type,amount,users,quantity) values('$date6','$purpose6','$type6',$amount6,'$user',$quantity6)");
        echo "<tr><td>$date6</td> <td>$purpose6</td> <td>$type6</td> <td>$quantity6</td> <td>$amount6</td> </tr>" ;
        $check=1;
    }
    if($date7&&$purpose7&&$type7&&$amount7){
        pg_query($con,"insert into credit (date,purpose,type,amount,users,quantity) values('$date7','$purpose7','$type7',$amount7,'$user',$quantity7)");
...
?>
            ...
            <h2>VIEW</h2><br><br>
               ...
               $con=pg_connect("host=localhost dbname=groups user=postgres password=postgres");
                    ...
                    $res=pg_query($con,"select distinct (purpose) from credit where users='$user'");
...
if(isset($_POST['view']))
    {
      ...
      $con=pg_connect("host=localhost dbname=groups user=postgres password=postgres");
            ...
            {
               $res=pg_query($con,"select * from credit where date between '$start' and '$end' and users='$user'");
           ...
           while($row=pg_fetch_array($res))
            ...
            {
             $res=pg_query($con,"select * from credit where date between '$start' and '$end' and type='$type' and users='$user'");
              echo'<table border="1" class="stable"><tr><th>Date</th><th>Purpose</th><th>Type</th><th>Quantity</th><th>Amount</th></tr>';
              while($row=pg_fetch_array($res))
               {
                 $viewdate=date("d-m-Y",strtotime($row[0]));
           ...
           {
              $res=pg_query($con,"select * from credit where date between '$start' and '$end' and purpose='$purpose' and users='$user'");
              echo'<table border="1" class="stable"><tr><th>Date</th><th>Purpose</th><th>Type</th><th>Quantity</th><th>Amount</th></tr>';
              while($row=pg_fetch_array($res))
              {
                $viewdate=date("d-m-Y",strtotime($row[0]));
       ...
       $res=pg_query($con,"select * from credit where date between '$start' and '$end' and purpose='$purpose' and type='$type' and users='$user'");
                    echo'<table border="1" class="stable"><tr><th>Date</th><th>Purpose</th><th>Type</th><th>Quantity</th><th>Amount</th></tr>';
                    while($row=pg_fetch_array($res))
                      {
                        $viewdate=date("d-m-Y",strtotime($row[0]));
