...
<html>
    ...
    <div class="header">
        ...
        <h1 class="heading">Fuel add or remove</h1><br>
        <div class="dgrid">    
...
$con=pg_connect("host=localhost dbname=groups user=postgres password=postgres");
    ...
    $res=pg_query($con,"delete from lube where name='$name' and price='$price'");
    if(pg_affected_rows($res) == 0){
        echo "Warning: Fuel Name=$name with Price=$price doesnt exists to remove";
    }
    else{
        echo "Successfully removed Fuel Name=$name with Price=$price";  
    }
}
...
$con=pg_connect("host=localhost dbname=groups user=postgres password=postgres");
    ...
    while($row=pg_fetch_array($res)){
        ...
        ||($row[0]==$name7 && $row[1]==$price7)){
            $check=1;
            echo "One of the fuel already exists";
            $lubecheck=1;
        }  
    } 
if($lubecheck==0){
    ...
    if($name0){
        pg_query($con,"insert into lube (name,price,size,quantity)  values('$name0',$price0,$size0,$quantity0)");
        echo "<tr><td>$name0</td> <td>$price0</td> <td>$size0</td> <td>$quantity0</td> </tr>" ;
        $check=1;
    }
    if($name1){
        pg_query($con,"insert into lube (name,price,size,quantity)  values('$name1',$price1,$size1,$quantity1)");
        echo "<tr><td>$name1</td> <td>$price1</td> <td>$size1</td> <td>$quantity1</td> </tr>" ;
        $check=1;
    }
    if($name2){
        pg_query($con,"insert into lube (name,price,size,quantity)  values('$name2',$price2,$size2,$quantity2)");
        echo "<tr><td>$name2</td> <td>$price2</td> <td>$size2</td> <td>$quantity2</td> </tr>" ;
        $check=1;
    }
    if($name3){
        pg_query($con,"insert into lube (name,price,size,quantity)  values('$name3',$price3,$size3,$quantity3)");
        echo "<tr><td>$name3</td> <td>$price3</td> <td>$size3</td> <td>$quantity3</td> </tr>" ;
        $check=1;
    }
    if($name4){
        pg_query($con,"insert into lube (name,price,size,quantity)  values('$name4',$price4,$size4,$quantity4)");
        echo "<tr><td>$name4</td> <td>$price4</td> <td>$size4</td> <td>$quantity4</td> </tr>" ;
        $check=1;
    }
    if($name5){
        pg_query($con,"insert into lube (name,price,size,quantity)  values('$name5',$price5,$size5,$quantity5)");
        echo "<tr><td>$name5</td> <td>$price5</td> <td>$size5</td> <td>$quantity5</td> </tr>" ;
        $check=1;
    }
    if($name6){
        pg_query($con,"insert into lube (name,price,size,quantity)  values('$name6',$price6,$size6,$quantity6)");
        echo "<tr><td>$name6</td> <td>$price6</td> <td>$size6</td> <td>$quantity6</td> </tr>" ;
        $check=1;
    }
    if($name7){
        pg_query($con,"insert into lube (name,price,size,quantity)  values('$name7',$price7,$size7,$quantity7)");
...
$res=pg_query($con,"select * from lube order by name asc");
